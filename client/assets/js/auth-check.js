...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof authSystem === 'undefined') {
        console.error('auth.js chưa được load. Vui lòng đảm bảo file auth.js được include trước file này.');
        return;
    }

    // Danh sách các trang cần đăng nhập (loại bỏ trangChu.html vì đây là trang đích)
    ...
    if (protectedPages.includes(currentPage)) {
        if (!authSystem.isLoggedIn()) {
            ...
            window.location.href = 'trangChu.html?showLogin=true';
            return; // Dừng thực thi các code phía dưới
        }
    }
    
    // Nếu đang ở trang chủ và có tham số showLogin=true, hiển thị modal đăng nhập
    if (currentPage === 'trangChu.html' && window.location.search.includes('showLogin=true')) {
