...
(function () {
  ...
  function normalizeImagePath(path) {
    // Nếu đang ở trong thư mục client thì bỏ bớt "client/"
  ...
  function renderCartPage() {
    ...
    cart.forEach((it) => {
      ...
      row.innerHTML = `
                ...
                <div class="--product-name">
                    <img src="${
                      normalizeImagePath(it.image) ||
                      "./assets/images/product-payment.png"
                    }" loading="lazy"/>
    ...
    footer.innerHTML = `
			<div class="pcl-footer-top">
				<div>
					Tổng thanh toán: <span class="pcl-t-f-price">${formatVnd(
            sumSelected(container)
          )}</span>
				</div>
			</div>
    ...
    container.addEventListener("click", (e) => {
      ...
      if (target.classList.contains("cartItem")) {
        ...
        const checkedCount =
          container.querySelectorAll(".cartItem:checked").length;
        if (checkboxAll) {
  ...
  function renderCheckoutPage() {
    ...
    try {
      selected = JSON.parse(
        localStorage.getItem(STORAGE_ORDER_BUFFER_KEY) || "[]"
      );
    } catch (e) {
      selected = [];
    }
    ...
    if (orderBtn) {
      ...
      orderBtn.addEventListener("click", (e) => {
        ...
        const addressInput =
          document.querySelector("#addressSpecial") ||
          document.querySelector("#address");
        const paymentRadio = document.querySelector(
          'input[name="py-type-radio"]:checked'
        );
        const paymentMethod = paymentRadio
          ? (function (idx) {
              return idx === 0 ? "COD" : idx === 1 ? "BANK" : "ONLINE";
            })(
              Array.from(
                document.querySelectorAll('input[name="py-type-radio"]')
              ).indexOf(paymentRadio)
            )
          : "COD";
        ...
        const order = {
          ...
          userId:
            (window.authSystem &&
              ...
              (window.authSystem.getCurrentUser() || {}).id) ||
            null,
          items: selected,
          total: total,
          address: address,
        ...
        const remaining = getCart().filter(
          (ci) => !selected.find((si) => String(si.id) === String(ci.id))
        );
        saveCart(remaining);
  ...
  function renderOrderSuccessPage() {
    ...
    const rightContainer = document.querySelector(
      ".payment-result-right .prr-content"
    );
      ...
      row.innerHTML = `
            ...
            "./assets/images/product-payment.png"
				...
				<div class="pr-item-main-content">
					...
					<div class="--product-qty pr-item-main-content-child">${
            Number(it.quantity) || 1
          }</div>
					<div class="--product-into-money pr-item-main-content-child">${formatVnd(
    ...
    const footerSubtotal = leftContainer.querySelector(
      ".pr-footer .pr-footer-item:nth-child(1) .pr-footer-item-content"
    );
    const footerDiscount = leftContainer.querySelector(
      ".pr-footer .pr-footer-item:nth-child(2) .pr-footer-item-content"
    );
    const footerTotal = leftContainer.querySelector(
      ".pr-footer .--product-into-money"
    );
    ...
    const codeEl = rightContainer.querySelector(
      ".prr-content-item:nth-child(1) .prr-content-item-content"
    );
    const timeEl = rightContainer.querySelector(
      ".prr-content-item:nth-child(2) .prr-content-item-content"
    );
    const totalEl = rightContainer.querySelector(
      ".prr-content-item:nth-child(3) .prr-content-item-content"
    );
    const methodEl = rightContainer.querySelector(
      ".prr-content-item:nth-child(4) .prr-content-item-content"
    );

    const createdAt = new Date(order.createdAt || Date.now());
    const pad = (n) => n.toString().padStart(2, "0");
    const timeString = `${pad(createdAt.getDate())}/${pad(
